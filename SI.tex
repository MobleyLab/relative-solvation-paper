...
The gradients from thermodynamic integration simulations for all four codes are summarized in Fig.~\ref{fig:ti_curves}.  The graphs are for the absolute transformation of methanol.  The gradients demonstrate how the different transformation functions especially the softcore potentials (see section~\ref{sec:softcores}) influence the TI gradients.  As can be seen these are very different implying that a lambda schedule must be appropriately planned for each simulation code and also the particular transformation.
...
\begin{figure}[ht]
  ...
  \caption{TI gradients obtained from various simulation codes for the absolute transformation of methanol. a) and b) AMBER charge and vdw transformations. c) Gromacs where red is from the Coulomb only transformation (but which does not make any contribution) and green for the free energy from all other contributions,  d) and e) SOMD charge and vdw transformations. f) CHARMM unified protocol.}
  \label{fig:ti_curves}
\end{figure}
