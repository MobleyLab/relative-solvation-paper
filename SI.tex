...
% arara: pdflatex
% arara: pdflatex
% arara: clean: {files: [SI.aux, SI.bbl, SI.blg, SI.log, acs-SI.bib]}
%
...
Table~\ref{tab:onestep} compares the separated protocol with the
...
\begin{table}[]
  ...
  \begin{minipage}{\linewidth}
    ...
    \begin{tabular}{llrrrrrr}
      \toprule
   &                & separated protocol &         &        & 1--step  &         &        \\
   &                & pmemd              & sander  &        & pmemd   & sander  &        \\
      transformation & & $\Delta G_\mathrm{sol}$ & $\Delta
                                                   G_\mathrm{vac}$ &
                                                                     $\Delta\Delta G$ & $\Delta G_\mathrm{sol}$ & $\Delta G_\mathrm{vac}$  & $\Delta\Delta G$ \\
      \midrule
      ...
      toluene        & methane        & 9.222              & 5.982   & 3.240  & 6.090   & 0.450   & 5.640  \\
      methane        & toluene        & -9.286             & -5.863  & -3.422 & -6.155  & -0.539  & -5.616 \\
      neopentane\footnote{\label{foot:cent}central mapping.}     & methane        & 70.163             & 69.848  & 0.315  & 65.763  & 58.495  & 7.267  \\
      methane\footref{foot:cent}        & neopentane     & -70.171            & -69.921 & -0.250 & -65.824 & -58.779 & -7.045 \\
      neopentane\footnote{\label{foot:term}terminal mapping.}    & methane        & 11.411             & 11.544  & -0.132 & 4.424   & 3.480   & 0.944  \\
      methane\footref{foot:term}        & neopentane    & -11.426            & -11.550 & 0.125  & -4.447  & -3.485  & -0.962 \\
      2-methylfuran  & methane        & 14.622             & 11.533  & 3.089  & 2.205   & -0.943  & 3.148  \\
      methane        & 2-methyfuran   & -14.604            & -11.504 & -3.100 & -2.216  & -0.063  & -2.153 \\
      2-methylindole & methane        & 24.251             & 15.473  & 8.778  & 7.113   & -4.021  & 11.135 \\
      methane        & 2-methylindole & -24.312            & -15.174 &
                                                                 -9.138
                                                                        & -7.128  & 1.858   & -8.986 \\
      \bottomrule
    \end{tabular}
...
and in vacuum (each $\Delta G$ is the sum of the charge and vdW plus
...
dummy atoms show $\Delta G$ and $\Delta\Delta G$ in line with the
separated protocol, the large transformations show in part striking
differences and even inconsistencies in forward and backward
simulations.  The latter is only evident in the vacuum simulations.
It is not clear however if the deviations can only be attributed to
the vacuum transformations.

Figure~\ref{fig:amber-dist} shows the average distance between the
carbon atom and the four attached hydrogens atoms in the neopentane to
methane.  The methane carbon atoms is mapped here to the central atom
...
with zero charges on the dummy atoms.
...
\caption{The average C--H distance shown as a function of $\lambda$
  ...
  lines display how the distance changes for the solution and the
  vacuum phase, and with and without explicit dummies.  The other test
...
of the c3--hc bond of the GAFF force field.  Tha crosses in violet
...
As Gromacs conveniently allows us use a separate $\lambda$ for bonded
...
expected given that the symmetrically arranged methyl groups (in terms
of the vdW parameters but with hydrogen charges on the vdW carbons)
will repel each other and thus not allow to reach the final distance.
Only after the final vdW only mutation are the final distances of
\SI{1.09}{\angstrom} reached.

In sum, this indicates a fundamental problem of the RAFE code in
AMBER.  Whether that is a bug or a conceptual issue with the
particular implementation can not be deduced at the moment.
