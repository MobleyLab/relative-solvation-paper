...
\caption{The mutation of ethane into methanol and explict topologies
  ...
  are the ones from the methanol end state c to ensures charge
  ...
  parameters as for methanol while dummy atoms (grey Du) have
...
switched on in opposite order if atoms are to be ``created''.

Figure~\ref{fig:dummies} shows how topology files may need to be
created in the case that the MD software does not allow independent
$\lambda$s for electrostatic and vdW mutations.  With Gromacs, for
instance, the transformation only requires a single topology file with
both A and B states (in single topology fashion, see main text) and a
single simulation control file with separate $\lambda$ vectors for
charge and vdW transformations.  Any intermediate state from
