...
However, from a technical point of view, all bond constraints in Sire/SOMD set all the bonds energies to zero. Such an approximation may be true for small and rigid molecules, but a systematic offset of about \SI{0.3}{kcal mol^{-1}}for each bond is introduced in perturbation such as methane to neopentane, where several carbon hydrogen bonds are transformed.
Considering neopentane to methane (centrally mapped) this bias gives a RAFE $\Delta$G=\SI{2.04 +- 0.01}{kcal mol^{-1}} contrasting the absolute calculations $\Delta$G=\SI{-0.190 +- 0.06}{kcal mol^{-1}}, as tab.~\ref{S-tab:sire_constraints} and fig.~\ref{S-fig:sire_constraints} show.
Furthermore, simulation of neopentane to methane (terminally mapped) shows a $\Delta\Delta G_{\mathrm{hyd}}$ =\SI{0.59 +- 0.01}{kcal.mol^{-1}} and $\Delta\Delta G_{\mathrm{hyd}}$ =\SI{-0.48 +- 0.01}{kcal.mol^{-1}} for the reverse path. As the free energy must be independent of the chosen path the discrepancy denotes a problem with using constraints on transformed atoms. 
...
\begin{equation}
 ...
 \Delta G= RT\ln ( \frac{r_{\mathrm{i}}}{r_\mathrm{f}} )  
\end{equation}
where $r_{\mathrm{i}}$ and $r_{\mathrm{f}}$ are the initial and final bond length respectively. Although for small bond lengths change this error may be negligible, it becomes pivotal for perturbations of  hydrogen atoms to carbon, nitrogen, oxygen or sulphur, where the bond length changes of several tenth of Angstrom

To test the goodness of unperturbed hydrogen bond constraints a comparison with no constraints simulations has been made. Figure SI shows that both protocols attains the same results between RAFE and absolute free energy estimations with a MUE between both predictions of XXX

The advantage of such a new protocl is to use still a good timestep and sa single topology apporach
namely with one perturbation/simulatoins Sire/SOmD si able to cgive as the ssame results as gromacs amber charmm
furthermore a comparison between reaciton field can be done
Now discuss about the fact that we can use a single step and results are compatible with gromacs and co
so we ndo not need further step s with the current technical implementation of the software
furthermore we could carried out a comparison with gromacs reaction field results
we are using bw their are using a generalized group based cutoff  [S-] to cite your SI
