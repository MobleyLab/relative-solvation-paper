...
%SB: I mean the entire functional form used in Gromacs:
%The way the softcore is implemented, or the functional form of the softcore itself
%could cause problems, which obliged you to use a two step protocol rather than one-step protocol.
%SB:
...
increase the time step to \SI{2}{fs}. 
 The split protocol was found to work  well for all codes (also CHARMM?).
...
Another question is the degree of usability of the softwares.
...
%JM suggests removing above paragraph, a bit long and speculative
...
%To achieve reproducibility in each code we have experimented 
%with different $\lambda$ schedules, and number and length of each %window.  We  also to not address the crucial ``time to answer'' %perspective because the MD  codes have different performance %characteristics, also depending on whether the  processor is CPU or %GPU, and the most efficient protocol is still an open 
%question, also due to the fact that some protocols don't seem to %work in all  codes as outlined above.
% JM suggests removing as does not add much
