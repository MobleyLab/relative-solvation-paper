...
be entirely duplicated i.e.\ for \texttt{sander} this means two topology files
with one end state each, and for \texttt{pmemd} both end states in one topology
...
\texttt{sander} but the data structures for mapped atoms are present only once.  The
...
by the developers, e.g. energy minimization in \textttt{sander} led to diverged
...
simulations have to be carried out separately with the \texttt{sander} program %DLM: Separately from what? Do you mean that if the solution calculations are done with PMEMD you have to do the vacuum calculations with sander anyway?
because \textttt{pmemd} cannot handle AFE simulations in vacuum at the moment.  This will, however, be rectified in future versions (Ross Walker, private communication).  A disadvantage of \texttt{sander} is that it cannot be used to simulate the $\lambda$ end points~\cite{doi:10.1021/ct400340s} such that the TI gradients need to be extrapolated (minimum and maximum allowed $\lambda$s are 0.005 and 0.995). 
%DLM: Wait, so this means that there's no way around extrapolating the endpoints in vacuum?
 Also, \texttt{sander} considers the whole system as the perturbed
region while \texttt{pmemd} restricts this to a user chosen atom selection.  This
...
molecule. 
\texttt{couple-lambda0} and \texttt{couple-lambda1} control the interactions of the molecule
specified by \texttt{couple-moltype} with its surroundings. For instance, if \texttt{couple-lambda0}
is set to \texttt{vdw-q} and \texttt{couple-lambda1} to \texttt{none}, the calculation will be set
...
In relative free energy calculations, however, \texttt{couple-lambda0} and \texttt{-lambda1} are
not necessary because \texttt{couple-moltype} should be set to \texttt{none}. 
