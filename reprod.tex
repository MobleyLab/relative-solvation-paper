...
%DLM: The explanation of the split protocol here doesn't mention what we're 
%doing for bonded interactions, whereas the unified protocol does. If we're 
%changing bond parameters at the same time as van der Waals (or charges) we 
%should say.
% HHL: we do it differently in AMBER and GROMACS, see sentence above and
...
for Lennard-Jones potentials with softcores.  Current CHARMM force fields,
...
potential for Lennard-Jones terms ($\alpha$=0.5 and $\sigma$=0.3) for
...
\begin{table}[]
  \begin{minipage}{\linewidth}
    \caption{Comparison of relative free energies of hydration for various MD 
      packages as obtained from relative transformations via unified or split 
      protocols.  Signs of the  backward transformation have been reverted to 
      correspond to the forward transformation.}\label{tab:relative}
    \makebox[\textwidth][c]{
      \begin{tabular}{llrrrrr}
        \toprule
        & & \multicolumn{2}{c}{AMBER\footnote{\label{foot:split}split 
        protocol.}} & 
        CHARMM\footnote{\label{foot:unified2}unified protocol.} & 
        GROMACS\footref{foot:split} & SOMD\footref{foot:unified2} \\
        & & \multicolumn{1}{c}{implicit\footnote{\label{foot:impl}using either 
        the implicit or the explicit dummy atom approach.}} & 
        \multicolumn{1}{c}{explicit\footref{foot:impl}} & & & \\
        \midrule
        ethane & methane & \num{0.02+-0.01} & \num{-0.13+-0.02} & \num{-0.11 +- 
        0.02} & \num{-0.04 +- 0.02} & \num{-0.01+-0.05} \\
        methane & ethane & \num{0.00+-0.03} & \num{-0.19+-0.03} & \num{-0.06 +- 
...
% HHL: I call it a bug for the moment whose origin we do not understmnd,
%      see Discussion.
...
CHARMM for alchemical free energy calculation (AFE) has been widely used with PERT module, but few bugs not previously reported in CHARMM c40b1 are found and careful AFE setup is needed to get robust and accurate result. Bugs regarding TI gradient accumulation on parallel version (not on serial one) were identified and fixed by Dr. Stefan Boresch.

PERT module does not allow hydrogen bond constraint (SHAKE) applied on
the perturbed region, and this requires end point lambdas to be equilibrated
carefully. These windows at end-point lambda were started with their own equilibration using \SI{0.1}{fs^{-1}} ~ \SI{0.5}{fs^{-1}} before the run. VSwitch option which is a switching function applied for potential is used, since PERT module is not compatible with VFSwitch option that is a switching function applied to force.

PSSP softcore potential function cannot handle Long-Range Correction (LRC) correctly. This effect is not clearly shown when initial state and the final state are comparable in size, but the deviation gets larger when they are not (e.g absolute alchemical free energy calcaultion). Only when LRC is turned off, $\Delta\Delta$G from AAFE and RAFE results will be well matched and accurate.

Figure~\ref{fig:LRC} shows the effect of turning LRC on/off and reasonable
cut--off setup. Top graph describes absolute solvation free energy of methane
based on LRC and cutoff values where switching function is effective
\SI{2}{\angstrom} before the cut-off value. Any value greather than
\SI{10}{\angstrom} of cutoff without LRC would be considered to reproduce
the results from other packages, and we employed \SI{12}{\angstrom} of cut-off
as suggested in CHARMM tutorial. Bottom graph shows relative solvation
free energy of 2--methylindole to methane from direct RAFE and from AAFEs
of methane and 2--methylindole with LRC on/off. The graph clearly shows
that LRC off is the key condition for self-consistency(when comparing RAFE
vs. difference in each AAFE) in CHARMM, and for the reproduciblitiy considering other packages showed ~\SI{8.7}{kcal.mol^{-1}} for the same calculation.
\begin{figure}[ht]
\includegraphics[scale=1.0]{figures/LRC}
\caption{(top) Absolute solvation free energy of methane based on LRC on/off
and different cut--off values. (bottom) Relative solvation free energy of
2--methylindole to methane from direct RAFE and from AAFE of methane
and 2--methylindole with LRC on/off. }
\label{fig:LRC}
\end{figure}
...
% GDRM: Update! Only LJ interactions in atoms whose parameters are being perturbed are in the soft core form 
...
% - Gromacs fails maybe for the softcore implementations
%DLM: Which softcore? You mean softcore Coulomb?
...
In this study we have addressed the question if the current MD packages AMBER, 
CHARMM, GROMACS and SOMD are able to reproduce relative alchemical free 
energies of hydration for a set of neutral small organic molecules given a 
pre--defined force field.  We have found that arriving at a simulation protocol 
that (mostly?) does this, has been a cumbersome due to technical difficulties 
encounted.  The MD codes have a wide range of options and setup features which 
makes it difficult for the user to decide on the most appropriate ones.
...
recommended for use with all four codes.  Unified protocols dp not appear to 
work universally for AMBER and GROMACS while SOMD and CHARMM(?) had no problem 
in this regard.  This may be a consequence of the different softcore functions 
used in these MD packages (see Eq.~S\ref{S-eq:general-softcore}x) but clearly 
needs further, detailled investifations.  We can therefore not recommed using 
this protocol without the user performing careful tests of their own.  The 
unpertubed H bond protocol is an interesting alternative that at least 
partially applies contraints on non--transforming bond and thus allows 
increasing the time step to \SI{2}{fs}.  The split protocol was found to work 
well for all codes (also SOMD/CHARMM?).  It appears to be the most efficient as 
shown with the methanol to methane case as the unified protocol 
produces a less smooth function~\cite{shirts_chapter_2007}. % HHL: @DLM what 
%are you really saying there?
A complete separation may not be necessary though and a certain degree of 
overlap between vdW and Coulumb $\lambda$ a viable 
solution~\cite{procacci_fast_2014} for equilibrium AFEs too. % HHL: is this 
%really true?  Procacci discusses this in the context of fast switching.

Comparison between codes is hampered by several factors.  There is the 
principal problem that the codes use different simulation algorithms.  But the 
data here suggest that, if there are any systematic errors introduced through 
this, then they are very small.  The electrostatics are handled differently in 
vacuum i.e.\ infinite cutoff vs.\ reaction field. % HHL: what else?

As part of this work we make our input data and protocols available.  These can 
serve as a test and benchmark set for new and future RAFE implementations but 
also to ensure reproducibility in current MD packages.  Running our test set 
will help to check for and avoid errors, etc.  For a complete high level system 
test we recommend to compare against results from absolute transformation.  The 
relative transformation should be run in both forward and backward 
transformation.  The results we have presented here serve as a benchmark 
developers can compare to.

Developers may be advised to look into various issues with current code bases.  
We have found that constraints in connection with varying bond length can cause 
problems with GROMACS just as masses must not be allowed to vary in RAFE 
simulations, both to avoid crashes and wrong results in the 
software.  CHARMM has issues with constraints and the PSSP softcores, and the 
PERT module cannot make use force switches as it is now standard for CHARMM 
force fields (refernce!).  AMBER's problem with end point geometries has been 
pointed out below.

Another question is the degree of usability of the softwares.  For example, 
when a mutation entails both appearing and disappearing parts in split 
protocols there is the problem of intermediates causing non--integral total 
...
needs to run two separate simulations per lambda because discharging and 
recharging groups cannot be selected separately.  In a unified protocol this 
is, however, not be a problem and it may be beneficial to go through an 
additional intermediate, e.g. indole in the case of the 2-- and 
7--cyclopentanylindole, to enhance efficiency.  % HHL: pure speculation without 
%merit :-)

Lambda paths as implemented in GROMACS would be beneficial for other codes as 
well to make the setup of split protocol much easier.  The current solution to 
this is essentially to mimick this protocol through careful constructions of 
topologies through scripting.  The setup of absolute free energy is nowadays as 
simple as instructing the MD software to mutation to ``nothing''.  For split 
protocols, however, stil extra care must be taken to ensure inclusion of all 
terms.  E.g.\ in the case of AMBER the solution for the charge transformation 
would be to set all charges of the end state to zero but this also means that 
intramolecule charge--charge contributions would not be calculated and thus an 
additional simulation in vacuo is required to correct for this.
...
investigated convergences as this would require further, complex 
investigations.  To achieve reproducibility in each code we have experimented 
with different $\lambda$ schedules, and number and length of each window.  We 
also to not address the crucial ``time to answer'' perspective because the MD 
codes have different performance characteristics, also depending on whether the 
processor is CPU or GPU, and the most efficient protocol is still an open 
question, also due to the fact that some protocols don't seem to work in all 
cases as outlined above.

We believe our study will improve the methodology and quality of relative 
alchemical free energy simulations.  Reproducibility is certainly a central 
corner stone of future applications.  However, RAFE simulation will need 
further improvements to progress the field and increase reliability.  One 
current concern is usability as alluded to above.  Further automation will 
surely enhance consistency and thus also reproducibility.  Various attempts in 
...
doi:10.1021/acs.jctc.6b00979}, including 
FESetup~\cite{loeffler_fesetup:_2015} which has been 
specifially designed to address RAFE simulation of small organic molecules.

\todo{%
  GPU: GROMACS, SOMD but not AMBER (yet) and
  CHARMM; alternative softcore functions?; sampling?; force field 
  improvements?; analysis e.g.\ BAR/MBAR?
}

\todo{discuss softcores and their improvements, discuss estimators, binding 
RAFEs?}


%Various estimators have been proposed to obtain the free energy from AFE 
%simulations.  Early work by Kirkwood~\cite{kirkwood_statistical_1935} 
%expresses 
...
%be sufficiently large~\cite{wu_phase-space_2005} the EXP approach typically 
%needs
...
%(MBAR)~\cite{shirts_statistically_2008}.  MBAR is a provable minimum variance
