...
alternative~\cite{doi:10.1021/acs.jctc.6b00794} considered molecules with a
...
changed along the alchemical transformations (this would affect only
...
The PERT module does not allow a hydrogen bond constraint (SHAKE) to be applied on the perturbed region, and this requires end point lambdas to be equilibrated carefully. These windows at end-point lambda were started with their own equilibration using \SI{0.1}{fs^{-1}} ~ \SI{0.5}{fs^{-1}} before the run. The VSwitch option was used to apply a switching function to the potential since VFSwitch option that is a switching function applied to force is not compatible with PERT module.
...
7-CPI & 2-CPI & \num{-0.01+-0.01} & \num{0.04+-0.01} &  \\
  ...
  \end{minipage}
\end{table}
While results from all three protocols (split, unified, absolute) seem to be in good agreement with each other, split-protocol result exhibits less standard error of the mean due to double amount of data. It is notable that split-protocol result is more similar to the ones from other MD packages (i.e. neopentane and toluene), but relative-unified result is more consistent with difference from CHARMM absolute simulation (2-methylindole, etc). This could be the result of AFE simulations done with unified protocol. Overall, the relative free energies obtained by three different protocols are in good agreement with other MD packages (Tables 1 and 3). 
...
use it. The data shown in Figure \ref{S-fig:gro_sc_eff} suggests that
...
the $\partial \mathcal{H}/\partial\lambda$, see Figure \ref{S-fig:gro_sc_eff}.
...
\begin{suppinfo}
All input files, setup, simulations and analysis of the dataset with
the above mentioned packages are freely available at
