...
that (mostly?) does this, has been cumbersome due to technical difficulties 
...
needs further, detailed investigations.  We can therefore not recommend using 
this protocol without the user performing careful tests of their own.  The 
unpertubed H bond protocol is an interesting alternative which 
applies contraints on all non--transforming bonds and thus allowed us to
increase the time step to \SI{2}{fs}.  The split protocol was found to work 
well for all codes (also SOMD/CHARMM?).  It appears to be the most efficient as 
shown with the methanol to methane case because the unified protocol 
produces a less smooth function~\cite{shirts_chapter_2007}.
...
As part of this work we make our input data and protocols available (see SI).  
These data can serve as a test and benchmark set for new and future RAFE 
implementations but 
...
test we recommend to compare against results from absolute transformations.  
...
PERT module cannot make use of the force switch as it is now standard for 
CHARMM force fields (refernce!).  AMBER's problem with end point geometries and 
unified protocols has been pointed out.
...
recharging groups cannot be selected separately.  Introducing an additional 
intermediate, e.g. indole in the case of the 2-- and 7--cyclopentanylindole, 
may enhance efficiency.  % HHL: pure speculation without 
%merit :-)

Lambda paths as implemented in GROMACS would also be beneficial for other codes 
to make the setup of split protocols much easier.  The current solution to 
...
intramolecule charge--charge contributions are calculated and thus an 
