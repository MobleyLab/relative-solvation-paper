...
%DLM: Also, the "from every lambda step" phrasing is unclear -- is the total 
%error 0.2-0.4 * nlambda, or ... ? ( You don't want to make the reader flip to 
%the SI to understand what you're saying.)
% HHL: Clarification: this is really only obvious in the 2-methylindole case.
%      Maybe there is something going on in the toluene case too but it is only
%      0.1 kcal/mol and so maybe hard to argue it is not just within
%      statistical error,
...
% HHL: discuss how happy we are with the CHARMM results/
...
well for all codes (also SOMD/CHARMM?) and appears to be the most efficient as 
shown with the methanol to methane case and AMBER as the unified protocol 
...
\todo{applicability to receptor systems?}

\todo{%
  what do we need to progress the field, usability e.g. automation to make 
  things easy but also consistent and thus more reproducible (FESetup also
...
\todo{%
  developer notes: constraint issues;
  both appearing/disappearing, usability: 
    charge balancing, discharge all to save on another recip PME evluation?, 
    2-cyclopentanylindole to 7-cyclopentanylindole: better to go through 
    intermediates?;
  lambda paths for AMBER (relative), AMBER absolute: crgmask requires;
  vacuum corr if split protocol
}

\todo{further investigation required: convergence, softcore, binding RAFEs?}
