...
% BR: This protocol turns a set of atoms into an ideal gas (no LJ, no bonds...). Is there any one who is rigorously this kind of annihilation? I thought maybe only Bill Jorgensen in the early days was doing this with Monte Carlo... It literally cannot be done with MD.
%JM: clarified that annihilation means the intramolecular non-bonded interactions are also turned off (and possibly torsions). We do that in SOMD. (RESOLVED)
...
% BR: We should introduce dual topology before single topology, as it is conceptually simpler. (RESOLVED)
...
%so suffer from the ``wandering'' ligand problem, as in absolute transformations\cite{Hermans-1986,Roux-bR-1996,GILSON19971047,Hermans-1997, doi:10.1021/jp0217839, deng_computations_2009}. (RESOLVED)
...
%JM propose to remove to avoid confusion, not sure what ''all the chemistry'' means
%But all the chemistry is now exclusively
%handled by, possibly, just a few non--core atoms such that this approach may only be of limited use.
...
% we don't have to go in that level of details (RESOLVED)
...
%DLM: What I had THOUGHT was the issue was that if your dummy atoms formed a closed loop, then you'd have problems, but otherwise the situation was OK.
%HL CAN YOU CLARIFY IF RESOLVED ?
%
...
%JM removed paragraph as that level of details is better discussed  when the AMBER implementation is explained. (RESOLVED)
...
%JM commented out this is mentioned later
...
% BR: Personally, I would not call PERT a dual topology. In fact, it is neither completely dual nor single. It depends on the user.(RESOLVED)
...
alternative mappings have been considered, see Figure~\ref{fig:cycles}.  One mapping has methane matched to a terminal methyl (green) and the other
...
\subsection{Free Energy Simulation Protocols}   %BR: NOT ONLY ABOUT RAFE IN THE FOLLOWING...(RESOLVED)
...
None of the atoms in the perturbed group where constrained and hence the time step set to \SI{1}{fs}.  
...
A few windows at the end-points (involving hydrogen being transformed to heavy atom or vice versa) were unstable due to steric clashes with starting coordinates and were equilibrated using \SI{0.1}{fs^{-1}} to \SI{0.5}{fs^{-1}}.
%
...
In this work we primarily focus on TI as this is supported by all the tested MD packages ``out--of--the--box''.
...
We will use absolute hydration free energies here as our standard point of comparison because for the present dataset these are easily calculated with high precision~\cite{doi:10.1021/acs.jced.7b00104} %check for more references
and are simpler to set up and implement than relative calculations.
...
precise{\color{red}Moved this to discussion to elaborate}
%{\color{blue}
...
TIP3P water density of \SI{0.980}{g/cm^3}~\cite{doi:10.1021/jz500737m}.
    ...
    %DLM: In terms of the density, should we note that this INCLUDES (i think) the contribution of the mass of the non-interacting dummy atoms? Specifically, it bothered me at first that, say, GROMACS does not give the same density for all boxes after the solute is disappeared, and that the density goes up with the size of the solute, but then I realized this is because the solute mass goes into the density calculation. In other words, if the final box ends up the same size the density will be higher because the solute is counted.
  %JM clarified in email how densities are evaluated  
\begin{table}[]
...
%This is an older table. I presume the above one has the newest
%CHARMM values
%\iffalse
%
%\begin{table}[]
% \begin{minipage}{\linewidth}
%    \caption{Absolute hydration free energies (in \si{kcal/mol}) and
%      end-state densities  (in g/cm3) as obtained from AFE
%      calculations. Uncertainties on the last decimal are given in
%      parenthesis.}\label{tab:absolute2}
%
%    \makebox[\textwidth][c]{
...
%JM resolved this as using previous CHARMM and GROMACS-reaction field values  
...
{\footnotesize
  \begin{minipage}{\linewidth}
    \caption{Comparison of relative free energies of hydration for various MD 
      ...
      protocols.  The values deduced from AFE transformations (given in the first row) were taken from Table 1. 
...
the closed cycle ethane$ \rightarrow$ methanol $\rightarrow$ methane $\rightarrow$ ethane (see Figure~\ref{fig:cycles}). The results are shown in Table~\ref{tab:cycle-closure}. Uncertainties were estimated by propagating uncertainties from the individual perturbations. AMBER explicit dummy is the only protocol consistent within uncertainty estimates, but the deviations observed with the other protocols are small. The largest discrepancy is observed with the GROMACS unified PME protocol, with the error just under \SI{0.2}{kcal/mol}.
...
absolute transformations, see Tables~\ref{tab:absolute2} and
...
Overall, the SOMD free energy estimations are in good agreement with the other MD packages, as the MAE suggests (see Table~\ref{tab:mue-relative}). For the methane $\rightarrow$ neopentane transformations SOMD yields consistent results between central and terminal mappings, as shown in Table~\ref{S-tab:sire-finalresults}
...
%
%, while GROMACS RF displays a $\Delta\Delta$G
...
%average SEM for absolute calcs in table 2 and aggregate number of MD steps per pert (1M = 1million)
% AMBER 0.014 kcal/mol 80M steps
% CHARMM 0.017 kcal/mol 31.5 M steps
% GROMACS 0.011 kcal/mol 200M steps
% SOMD 0.045 kcal/mol  34M steps

%average SEM for relative calcs in table 4
% AMBER implicit 0.024 kcal/mol 80M
% AMBER explicit  0.027 kcal/mol 80M
% CHARMM 0.015  kcal/mol  31.5M
% GROMACS 0.020 kcal/mol  197.4M
% SOMD 0.048 kcal/mol 17M
