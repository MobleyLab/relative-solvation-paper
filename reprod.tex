...
{\color{red} Do we need to explain ``rigorous'', i.e., methods giving asymptotically correct free energy estimates (correct in the limit of enough simulation time) given a choice of model?}
...
%need to define ``decoupling''? Will we also be using ``annihilation''? If so may have to clearly define both when first used.
...
%Seems like parallel/rectangular might not be the best way to say this, as one could presumably draw a trapezoidal thermodynamic cycle and it could still work as well.
...
mutated.  
%"Description" seems vague. Set of coordinates? Should we have a figure explaining the difference?
Thus, atom types are directly transformed into the new type,
typically by linearly scaling force field parameters.
Disappearing/appearing atoms are balanced with ``dummy'' atoms, %"balanced with" -> "represented by"?
...
cancel in the thermodynamic cycle. 
%DLM: I don't understand what this is saying about how AMBER does things. Clarify?
 RAFEs are useful e.g.\ in ranking
...
context of relative free binding energies between small molecules and
