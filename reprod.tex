...
%DLM: Not sure I like the use of quotes in the following paragraph to introduce terms, as it seems like they are used around terms which haven't been defined but without defining those terms. If anything the quotes make it harder to read because they highlight that the terms have not been defined. I suggest either avoiding the quotes and skipping definitions, or using quotes (or italics) and making sure each term is clearly defined. 
...
The method has been applied in various forms for several decades now since
...
%DLM: I think the point you are trying to make with those two sentences (which I don't like as written) is this: "This approach can be particularly helpful and easy to implement as compared to approaches which involve transitioning between states via a physical pathway. It can be harder to implement and automate physical pathway methods, and such methods can also require crossing of free energy barriers which are prohibitively large if pathways are not well chosen. " I would get rid of "This can be a ..." and "For instance...", both, and replace with something along these lines.
...
%DLM: There's that double quotes to introduce a term without giving a definition, again.
...
%DLM: quotes/wandering. Again, I think if we are going to make clear we are introducing the term (such as by putting it into quotes) we need to define it. Otherwise, we don't indicate it is new and say something like, "See also the wandering ligand problem, discussed below.)
...
But also chemical similarity is also of importance; e.g.\ chirality and binding modes 
where the relative three dimensional arrangement of groups in space must be taken into 
...
%DLM: I don't follow the last three sentences of this. What does it mean that "the corresponding atom changes were made equal"? I'm not quite getting how this is different from single topology.
...
GROMOS~\cite{doi:10.1021/jp984217f} and SOMD~\cite{Sire-2016,
  doi:10.1021/ct300857j}, often provides
a hybrid single/dual topology approach
...
Another study went further and also compared densities across packages, revealing a variety of issues~\cite{Schappals:2017:J.Chem.TheoryComput.}.
For free energies, given a
predefined force field and run--time parameters we ought to be able to
obtain comparable free energy results within statistical convergence
limits, though this has not yet been tested.
...
them~\cite{Skyner:2015:PCCP}.  They are also needed for calculations of a variety of important physical properties, and to calculate
binding free energies where the simulation in solution (see
Fig.~\ref{fig:thermocycle}) is combined with a mutation of the
molecule bound to a partner~\cite{Skyner:2015:PCCP}.  A large database of hydration
free energies computed from alchemical free energy (AFE) simulations, FreeSolv, 
...
%DLM: The explanation of the split protocol here doesn't mention what we're doing for bonded interactions, whereas the unified protocol does. If we're changing bond parameters at the same time as van der Waals (or charges) we should say.
  The order in which this has to be done is detailed in section~\ref{S-sec:separated}x of the SI.  In the split protocol, the scaling of the bonded terms can be combined with either transformation.
...
linearly scaling back these interaction potentials as a function of an interaction parameter, $\lambda$, causes these functions to increasingly behave like
hard--sphere potentials as $\lambda\rightarrow 0$.  This implies a
%DLM: I am not sure this is clear. Hard spheres can't interpenetrate, so a reader would probably ask, "Wait, why does this imply they are likely to interpenetrate?" Maybe a better phrasing would be, "This means that there is a discontinuous change between $\lambda=0$ where particles can overlap, and $\lambda=\epsilon$ (where $\epsilon$ is very small) where particles still behave like hard though minuscule spheres. This can lead to strongly fluctuating..."
higher probability of other atoms penetrating into the highly
...
latter case, the total energy term resulting from the potential is scaled.
%DLM: Unclear "the total energy term resulting from the potential..." Do you mean, "In the latter term, the potential energy term for that specific interaction is scaled." (?)
The two approaches are not mathematically equivalent and so the pathway through state space or alchemical space 
...
%DLM: I disagree with this somewhat:, as I still prefer to use scaling for Coulomb and softcore for LJ only, so I think this should say: 
It should be noted, however, that modification of Lennard-Jones interactions is handled through softcore functions in modern codes (as is, in some cases, that for electrostatic interactions) and thus parameter scaling~\cite{doi:10.1021/j100056a020, JCROSS1986198} is now only used for bonded terms and sometimes electrostatic interactions, at least in modern codes. 

One more important issue is whether the code allows constraints to be applied to bonds which have 
...
%DLM: Last sentence seems out of place.
...
%DLM: Wait, set to the mass of the heavy atom? I don't understand. Are you saying that all hydrogens being perturbed get the mass of the heavy atom they are connected to? Can you clarify why? (May also want to say that it doesn't matter!).
...
All simulations were started from simulation boxes prepared by FESetup~\cite{loeffler_fesetup:_2015}.  It should be noted, 
...
%DLM: I don't understand the last sentence. 
...
transformation from a smaller to a larger solute, water molecules may be in particularly
close proximity to the solute.  The production simulations were run at 
...
appropriate for the final simulation temperature.  In a very few cases it was necessary to use coordinates from a nearby $\lambda$ state because of simulation instabilities. 
%DLM: Do we mean "coordinates from the end of the simulation at a nearby $\lambda$ state? Otherwise, since all of the starting coordinates are identical, this would make no difference, right?
This happened in transformations with a larger number of dummy atoms.  Water hydrogens (TIP3P) were constrained with SHAKE but none of the atoms in the perturbed group
%None of the atoms in the perturbed group what? "None of the atoms were in the perturbed group"?? Not sure what's meant.
 and so the time step was set to \SI{1}{fs} (see SOMD protocol below for an alternative approach).  The temperature was controlled through a Langevin thermostat with a friction constant of \SI{2.0}{ps^{-1}} and pressure rescaling through a Monte Carlo barostat with 100 steps between isotropic volume change attempts.  Long--range electrostatics in solution was handled with PME and a cutoff of \SI{8.0}{\angstrom} for both Coulomb and vdW interactions.  In vacuum no cutoff was used.
...
%% JM which ones? What was the reason for doing so? Same as AMBER? DLM: Agree with these questions.
...
16 states, respectively, and simulated for \SI{4.2}{ns} at each $\lambda$ value with time steps of 
\SI{1.0}{fs} in water and vacuum. 
%DLM: 1 fs for the absolute calculations, too? Or did those use two fs? And where do we say how many lambda values used for the absolute claculations/ Your sentence only addresses the first two cases, there is no sentence on the third case.
Atomic masses were not changed along the 
alchemical path;
...
%DLM: What is m_atom? Can't use symbols without defining. 
...
%DLM: Guilherme, you have to verify, but I think you're saying you used 1 fs timesteps because you didn't constrain hydrogen bonds. So then you would say, "Hydrogen bonds were left flexible" or some such.
...
%DLM: This is important; please explain, GDRM. 
...
%DLM: I agree with these comments. Also note I think it's a provably minimum variance estimator, which is different from "lowest variance of any known estimator"
...
%DLM: What is this last statement based on? GROMACS DOES couple these to lambda unless you tell it not to (hence our use of "mass-lambdas" below), AND it evaluates the gradients and output them, it's just that we could ignore them in our analysis (though here we told it not to change the masses).
...
data.
%DLM: "can be" doesn't make clear whether we did that; did we? 
...
\SI{0.2}{kcal.mol^{-1}} for CHARMM/GROMACS, and CHARMM/AMBER. %DLM: Would be good to have error bars on those.
...
\begin{table}[]
  \begin{minipage}{\linewidth}
    \caption{Comparison of relative free energies of hydration for various MD 
      packages as obtained from relative transformations via unified or split protocols.  Signs of the  backward transformation have been reverted to correspond to the forward  transformation.}\label{tab:relative}
    \makebox[\textwidth][c]{
  ...
  \end{minipage}
\end{table}
%DLM: Make clear somehow in the table caption what "implicit" vs "explicit" is; naively I would read this as implicit vs explicit solvent but presumably that's not what's meant.
...
% HHL: move the following to the general results and compute for all packages
...
%DLM: Maybe we should say, "We suspect these issues reflect a bug in the AMBER package but have been unable to isolate it; we have reported the issue to the AMBER developers," or something similar. 
%DLM: Also, the "from every lambda step" phrasing is unclear -- is the total error 0.2-0.4 * nlambda, or ... ? ( You don't want to make the reader flip to the SI to understand what you're saying.)

\subsection{CHARMM}
\label{sec:charmm-results}

% HHL: Donghyuk, please fill in!
% DLM: Have we been in touch with him and Benoit, or are we removing?
...
Using this in relative calculations is possible, but will result in unintended behavior and errors. %DLM: Added this -- GDRM, OK/accurate? I think we want to make the point not just that this is useful for absolute but that you'd better NOT use it for relative, as I believe this caused some of your issues?
  \inpopt{couple-lambda0} and 
...
%sigma).
% DLM: GDRM, you're going to have to check the manual and users lists. The question is, when you modify the LJ interactions between atoms using the soft core functional form, are you modifying ALL LJ interactions, or only those between atoms whose parameters are being perturbed? My vague recollection is that it is 
% between all atoms, but this may only have been an older version of the setup. If you still don't understand the question, come talk to me.
%
...
%DLM: Uncertainties should never be zero; one needs additional precision when this is the case (though I realize it does not look as nice).
...
potentials. 
%DLM: From GROMACS? Needs to say.
Signs of the backward transformation have been 
  reverted to correspond to the forward transformation. The complete version 
...
\end{tabular}
\end{table}
%DLM: Uncertainties should never be +/-0.00; you need more precision if that's the case.
...
%DLM: Which data? "Our data"? But specifically which data? Maybe you mean, "The data shown in Figure () suggests that..."
...
%DLM: Missing table reference here
...
% - Gromacs fails maybe for the softcore implementations %DLM: Which softcore? You mean softcore Coulomb?
...
%DLM: Table reference isn't showing up
...
%DLM: Maybe we say that it indicates these are unreliable and the issue will need further investigation; at present we do not recommend these
...
%DLM: Seems like the exception to that is CHARMM, which looks like it is just consistently wrong. 
...
\todo{SHAKE/LINCS issues with changing bond lengths}
