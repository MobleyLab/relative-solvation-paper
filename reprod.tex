...
%JM: clarified that annihilation means the intramolecular non-bonded interactions are also turned off (and possibly torsions). We do that in SOMD.
...
% JM  in some codes there are two sets of coordinates but I think
% we don't have to go in that level of details
...
%JM removed paragraph as that level of details is better discussed  when the AMBER implementation is explained.
%Dummy atoms are typically explicitly listed in topologies as atoms %with null nonbonded parameters. AMBER is a notable exception %where the user only flags the  disappearing and appearing 
%atoms in topologies of the respective end-states in the control 
%file of the MD engine.  
%
%The contributions of bonded terms that involve at least one %dummy atom are not factored into the free energy 
...
scaling, integrators, etc).  In addition there may be difference in the  choice of physical constants used for evaluating potential energies. A previous study noted that variations in the hardcoded values of Couomb's constant lead to detectable differences in single point energies calculated by CHARMM, AMBER or GROMACS.\cite{Shirts2017,SOMDcoulomb}  
...
\progname{sander} this means two topology files with one end state each, and for \progname{pmemd} both end states in one topology file. 
 ...
 The code loads two separate input topologies that describe the end states of interest and allows users to map atoms between the two end-states that will share the same coordinates for the free energy calculation. Evaluation of the interactions involving these atoms as a function of the coupling parameter is done by default via  linear scaling of the energy and forces of the end-states. Alternatively the user can request that softcore potential be used. The non-bonded interactions of atoms that are not paired between the end-states are handled with a softcore potential. In addition, bonded terms involving different unpaired atoms are ignored. This in effect amounts to defining unpaired atoms as dummy atoms in one of the end-states. We call this the ''implicit dummy protocol'' since the procedure is handled automatically by the software through analysis of the end-state topologies rather than via explicit definition of dummy atoms in an input topology. 
...
Alternatively it is possible for the user to construct an input topology of a single molecule that explicitly contains dummy atoms such that the desired end-states can be simulated. This is a similar approach to what SOMD and GROMACS do and we call this the ''explicit dummy protocol''. 
...
interactions.  In vacuum no cutoff was used.  {\color{red} Specify the number of windows and the length of each simulation of each window. Clarify whether a LRC correction term was used for the truncated VdW interactions during the simulations. If not, clarify whether a LRC VdW correction term added by post-processing of the end-states.}
...
%
The PERT module can not currently not use force switching (option \inpopt{VFSwitch}) for LJ potentials with softcores.  Current CHARMM force fields, however, are parameterized to use force switching. % citation needed
In this work we have used potential switching only (option \inpopt{VSwitch}).{\color{red} clarify the value of the inner cutoff used with the potential switching}
...
{\color{red} clarify whether  a VdW LRC term was used in the simulations, if not was an end-point correction made?}
...
Additionally, an end-point correction for truncated VdW potentials was applied as described previously elsewhere.~\cite{shirtsLRC,BosisioHG}
...
The same setup for relative free energy calculations was followed to compute absolute free energies of hydration. Thus, 17 evenly spaced $\lambda$ windows were used and simulations were run for \SI{2}{ns} employing a velocity-Verlet integrator with \SI{2}{fs} time step, constraining only the hydrogen bonds which are not alchemically transformed. 
...
%JM I have toned down above claims to address comment
...
The  precision of the calculated free energies is similar between AMBER, CHARMM and GROMACS, whereas the SOMD free energies are  ca. 3 fold less precise. This may reflect differences in the lambda schedules and length of trajectories between the different codes. Nonetheless the standard errors are typically well under 0.1 kcal/mol, thus it becomes meaningful to investigate small differences of a few tenth of kcal/mol between codes. 

The $\Delta\Delta G_{\mathrm{hydr}}$ 
...
well.  GROMACS predicts a smaller $\Delta\Delta G_{\mathrm{hydr}}$ for the methanol transformations by about \SI{+0.2}{kcal.mol^{-1}}.  Similarly, there are some small discrepancies in the toluene, 2--methylfuran and 2--methylindole cases, where CHARMM produces slightly smaller $\Delta\Delta G_{\mathrm{hydr}}$.
...
%JM I think is is addressed in the text now
...
%JM TODO compute uncertainties on MAD for Table4
% xsmake plot instead of Table
...
G_\mathrm{hydr}$ of \SI{0.07+-0.04}{kcal.mol^{-1}} and for the explicit dummy 
simulation the error is \SI{-0.02+-0.05}{kcal.mol^{-1}}.
...
(unified protocol).
{\color{red} DK and SB to report cycle closure errors for CHARMM and SOMD}
...
%JM fix broken referencing to SI
...
%JM I think this comment is resolved
...
%JM fix broken reference to SI table
...
Figure \ref{S-fig:gro_sc_eff}. A variant that combined the bonded terms with the vdW transformation did not change this result.  Thus, we find that the split protocol without Coulomb softcore potentials is the most effective way to calculate relative free energies with the current GROMACS implementation.
...
%or the use of \inpopt{constraint = 
%none} and of explicit constraints in the topology file.
%JM suggests removing above sentence as it is relies on a detailed knowledge of GROMACS input files.
...
to/from a heavy atom. This problem has already been discussed by Pearlman et al.~\cite{pearlman1991overlooked} and Boresch et al.~\cite{doi:10.1021/jp981628n, doi:10.1021/jp981629f}.

%, as a missing bond--stretching term
%JM
% do we need to mention this equation? We don't use it 
%anywhere in the manuscript as far as I can tell
%, which introduce a free energy offset estimated as:
%\begin{equation}
...
The free energies we have computed appear to be in reasonable good agreement with each other (see Tables~\ref{tab:absolute} and \ref{tab:relative}).  The MAE is \SI{0.2}{kcal/mol} for absolute free energies and \SI{0.3}{kcal/mol} for 
...
reaction field.  Temperature and pressure control, time step integrators, etc. are other examples.  But the data here suggest that, if there are any systematic errors introduced through these algorithms, then they are very small.  It is reassuring that AFEs for the systems tested here show only a small dependence on MD protocol decisions (provided a correct implementation).

As part of this work we make our input data and protocols available (see SI).  We recommend to use this dataset to test and benchmark future RAFE implementations to validate reproducibility against other simulation packages.
...
%JM do we need the paragraph below. It is rather hard to follow, it seems to me that this is captured by the above sentence 'careful construction of topologies'.
%The setup of absolute free energy is nowadays as simple as %instructing the MD software to mutate to ``nothing''.  
%For split protocols, however, extra care must be undertaken to %ensure inclusion of all terms.  E.g.\ in the case of AMBER this %means to set all charges of the end state to zero for the charge %transformation step. This also means that 
%intramolecular charge--charge contributions will all be zero, and %an additional simulation in vacuo is needed to correct for this.
%

The primary focus of this work was to achieve low statistical errors to establish if codes are able to reproduce free energies.  We have not investigated rate of convergences as this would require further, complex investigations.  
