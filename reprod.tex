...
that (mostly?) does this, has been a cumbersome and long--winded process.  The 
MD codes have a wide range of options and setup features which makes it 
difficult for the user to decide on the most appropriate ones.
...
\todo{%
  recommended protocol, protocols to avoid;
  lessons learned;
  applicability to receptor systems?;
  SHAKE/LINCS issues with changing bond lengths -> unperturbed hbonds;
  split vs unified (both relative and absolute)
}

\todo{%
  We can't do convergence tests so only discuss and explain why not done: 
  different lambda schedules, number of windows, length of each
  windows; codes have different speeds  (``time to answer''). Instead focus 
  is on low errors to establish if codes are different or not.
}
