...
some parameters (split protocols, see below) must be emulated through 
...
a few dummy atoms are involved while the protocol works for the smaller 
transformations (refer to Figure~\ref{fig:cycles}).  The split RAFE 
protocol and the absolute free energies, however, are very close to the other MD packages as demonstrated in Tables~\ref{tab:absolute} and \ref{tab:relative}.
...
rapidly varying part of $\partial\mathscr{H}/\partial\lambda$, see Figure
...
simulations will blow up if a hydrogen alchemically becomes a heavy atom and the simulation employs hydrogen bond constraining algorithms such as SHAKE or LINCS. Successful simulations require turning off the constraint and decreasing 
...
other MD packages, as the MAE suggests (see Tables~\ref{tab:absolute} and \ref{tab:relative}).  Reaction field and PME results are in good agreement too.  In particular, the unperturbed hydrogen bonds constraint protocol 
%DLM: "unperturbed hydrogen bonds constraint" is unclear.
%JM: suggestions for a better name?
allows a timestep of \SI{2}{fs}.

All SOMD RAFE simulations were carried out with simultaneous transformation of Lennard-Jones, charges, and bonded terms i.e.\ the unified protocol.  This suggests that the failure of GROMACS in some instances may be due to differences in the softcore implementations.
...
%affect their
% protocols
As regards the methane $\rightarrow$ neopentane transformation, SOMD is able to 
maintain consistency between central and terminal mapping, as shown in 
Table~\ref{S-tab:sire-finalresults}, while GROMACS RF displays a $\Delta\Delta$G of \SI{-0.20+-0.01}{kcal.mol^{-1}} for the centrally mapped transformation and $\Delta\Delta$G = \SI{-0.05 +- 0.02}{kcal.mol^{-1}} for the terminally mapped transformation.  The SOMD RF calculations show a MAE of
\SI{0.18}{kcal.mol^{-1}} and MAE of \SI{0.23}{kcal.mol^{-1}} with GROMACS PME 
