...
%JM If you recall JM we modified FESetup such that for SOMD input topologies 
% we have the atomic masses of the heaviest end-state.   
...
below for an alternative approach that was subsequently tested).  The temperature was controlled through a 
...
%JM add details about electrostatics and cutoffs for the solvated and vacuum phases. 
...
Thus, we compare relative free energies calculated via four distinct protocols. The first features simultaneous parameter change simulations (``unified protocol''). The second features separated parameter change simulations (``split protocol''). The third is obtained from differences of two absolute hydration free energies, which are themselves produced by a unified (``absolute-unified protocol'') or split (``absolute-split protocol'')  protocol depending on the package used. For some packages, several variations of such protocols were tested before comparing the results with other packages. The present section uses data produced by a 'recommended' protocol for a given package, and details about additional protocols are given in sub-sections below.

The unified protocols change partial charges, van der Waals parameters, and 
...
hydration obtained using the absolute-unified or absolute-split protocols. Overall the $\Delta\Delta G_{\mathrm{hydr}}$ 
obtained with the various MD packages agree to within 0.1--\SI{0.5}{kcal.mol^{-1}}. Notable discrepancies include  GROMACS that predict a smaller $\Delta\Delta G_{\mathrm{hydr}}$ than the other packages for the methanol transformations by about \SI{0.2}{kcal.mol^{-1}}.  Similarly, there 
