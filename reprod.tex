...
Generally, the Sire/SOMD free energy estimations are in good agreement with all the other codes, as the MUE suggests. Reproducibility was attained between reaction field and PME methods. In particular, the unperturbed hydrogen bonds constraint allows a timestep of \SI{2}{fs} and all the Sire/SOMD simulations can be carried out in one single step, with a single-topology paradigm, differently from multi-step approaches used in Amber, Gromacs and CHARMM.
...
\begin{table}[]
  \begin{minipage}{\linewidth}
  \caption{MUE comparison between Sire/SOMD, Gromacs with Reaction Field (Gromacs RF), Gromacs with PME (Gromacs PME) and Amber in terms of MUE (\SI{}{kcal.mol^{-1}}.}\label{tab:muebound-comp}
  \makebox[\textwidth][c]{
  \begin{tabular}{lcccc}
    \toprule
    Package & Sire/SOMD & Gromcs RF & Gromacs PME & Amber\\
