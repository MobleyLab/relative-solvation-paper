...
encountered.  The MD codes have a wide range of options and setup features 
...
simulation has currently be done with the separately developed sander module.  
The problem may be a consequence of the different softcore functions 
(see Eq.~S\ref{S-eq:general-softcore}x) used in these MD packages but clearly 
needs further, detailed investigations.  We therefore can not recommend 
this protocol without the user performing careful tests of their own.
...
well for all codes (also SOMD/CHARMM?).  It appears to be the most efficient 
...
principal problem that the codes use different simulation algorithms.  E.g.\ 
the electrostatics are handled differently in vacuum i.e.\ infinite cutoff vs.\ 
...
systematic errors introduced through these, then they are very small.  This 
also reassures us that AFEs are only little dependent on MD protocol decisions, 
provided they are sane, and actual implementation of algorithms.
...
will help to check for and avoid errors.  For a complete high level system 
...
Developers are advised to look into various issues with current code bases.  
...
CHARMM force fields (reference!).  AMBER's problem with end point geometries 
...
may be an interesting choice of path way and possibly be of reasonable 
efficiency.  % HHL: pure speculation without %merit :-)

Lambda paths as implemented in GROMACS would also be beneficial for other codes 
as they make the setup of split protocols much easier.  The current solution to 
...
corner stone of future applications. % HHL: we only discuss a subest of 
%reproducibiliy
  However, RAFE simulation will need 
