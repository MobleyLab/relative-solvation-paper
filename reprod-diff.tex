...
%DIF DEL /home/hhl/Chem/Reproducibility/JCTC/reprod.tex   Wed Aug 29 21:12:06 2018
%DIF ADD reprod.tex                                       Sun Sep 16 18:27:41 2018
...
\providecommand{\DIFdel}[1]{\protect\cbdelete{\protect\color{red}\sout{#1}}\protect\cbdelete} %DIF PREAMBLE
...
\begin{abstract}
  ...
  important modern simulation technique \DIFaddbegin \DIFadd{to calculate free energy changes on binding or solvation}\DIFaddend .  Contemporary molecular
  ...
  free energy \DIFaddbegin \DIFadd{differences }\DIFaddend should be obtained within statistical bounds regardless
  ...
  codes.  \DIFaddbegin \DIFadd{Absolute alchemical free energy simulations have been
  carried out as a reference.  }\DIFaddend Achieving this level of reproducibility
...
Torsional interactions may also be scaled in an annihilation protocol, but bond and angle terms are usually not scaled as this leads to poorly converging free energy \DIFdelbegin \DIFdel{changes}\DIFdelend \DIFaddbegin \DIFadd{estimates}\DIFaddend .~\cite{doi:10.1021/jp981628n}
...
biomolecules~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{doi:10.1021/ja512751q,
  doi:10.1021/acs.jctc.6b00991}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{christ_accuracy_2013, doi:10.1021/ci5004027,
  doi:10.1021/ja512751q, doi:10.1021/acs.jctc.6b00991}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

RAFEs can be calculated by making use of either the so--called single or dual topology method.
%DIF <  BR: We should introduce dual topology before single topology, as it is conceptually simpler. (RESOLVED)
...
%DIF < BR: This has some similarity with the wandering ligand problem, but adding too much here may lead to confusion.
%DIF < so suffer from the ``wandering'' ligand problem, as in absolute transformations\cite{Hermans-1986,Roux-bR-1996,GILSON19971047,Hermans-1997, doi:10.1021/jp0217839, deng_computations_2009}. (RESOLVED)
...
%DIF > Single topology means that there is only one connected representation of the molecule to be transformed into another molecule.
%DIF >  BR:
Single topology means that \DIFdelbegin \DIFdel{there is only one connected representation of the molecule to be transformed }\DIFdelend \DIFaddbegin \DIFadd{the alchemical transformation of one molecule }\DIFaddend into another molecule \DIFdelbegin \DIFdel{.
%DIF <  deleted: also implying a  single set of coordinates.
%DIF <  BR This is not true: in dual topology too there is a single set of coordinates
%DIF <  JM  in some codes there are two sets of coordinates but I think
%DIF <  we don't have to go in that level of details (RESOLVED)
}\DIFdelend \DIFaddbegin \DIFadd{is handled via a single set of connected atoms.
}\DIFaddend Atoms of a given type are directly transformed, typically by linearly scaling the force field parameters, into atoms of a different type.
...
\DIFdelbegin \DIFdel{In }\DIFdelend %DIF >  BR:
\DIFaddbegin 

\DIFadd{An example of single topology is the simple transformation of methane (CH4) into tetrafluoromethane (CF4), 
in which real atoms are mapped into real atoms with different force field parameters at the two end states.
%DIF > In typical implementations,
However, in most }\DIFaddend typical implementations,
...
\DIFaddbegin \DIFadd{For example, one dummy atom is necessary to match a methane molecule (5 atoms) transforming into methanol (6 atoms).
}\DIFaddend Dummy atoms have no non--bonded interactions in the end state but normally retain the bonded
terms of the original atom to avoid complications with unbound
atoms~\cite{doi:10.1021/jp981628n}.
\DIFdelbegin \DIFdel{However, it is important to }\DIFdelend %DIF >  but this is somewhat controversial in the literature~\cite{Boresch:2002:Mol.Simul.}.
\DIFaddbegin \DIFadd{Some practitioners }\DIFaddend stress that a \DIFaddbegin \DIFadd{given }\DIFaddend dummy atom should retain at most only one angle term \DIFdelbegin \DIFdel{(Atom1--Atom2--Dummy) and one dihedral term
(Atom1--Atom2--Atom3--Dummy) }\DIFdelend with respect to \DIFaddbegin \DIFadd{``real'' }\DIFaddend non-dummy atoms to yield correct results\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{doi:10.1021/jp981628n,doi:10.1021/jp994193s}}\hspace{0pt}%DIFAUXCMD
.  %DIF < {color{red} JM: HL mark as resolved discussion in comments in the tex source if the issue is resolved }
%DIF < DLM: I don't actually understand why this is; clarify slightly? What's wrong with having a second angle term of a second dihedral? And what exactly is meant by "angle term"? e.g. if I have a benzene proton I am turning into a dummy atom, it has two different angle terms running in different directions around the ring; do these comprise two different "angle terms"?
%DIF < DLM: What I had THOUGHT was the issue was that if your dummy atoms formed a closed loop, then you'd have problems, but otherwise the situation was OK.
%DIF < HL CAN YOU CLARIFY IF RESOLVED ?
%DIF < 
%DIF <  BR: I cannot make sense of the rest of the paragraph. Also, you seem to ascribe this method only to AMBER and yet one reference is from me (with CHARMM).
%DIF < JM removed paragraph as that level of details is better discussed  when the AMBER implementation is explained. (RESOLVED)
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{doi:10.1021/jp981628n,doi:10.1021/jp994193s,Boresch:2002:Mol.Simul.}}\hspace{0pt}%DIFAUXCMD
.  For example, if the angle term Atom2--Atom1--Dummy1 is included then additional angle terms such as Atom3--Atom1--Dummy1 should not be included.  Likewise, if the dihedral term Atom3--Atom2--Atom1--Dummy1 is included then additional terms such as Atom4--Atom2--Atom1--Dummy1 should not be included.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Another algorithmic decision for single topology is whether  the implementation scales force field parameters (``parameter
scaling'') and/or energy components (``energy
scaling'')~\mbox{%DIFAUXCMD
\cite{doi:10.1021/jp981628n}}\hspace{0pt}%DIFAUXCMD
.  In the former case each parameter is
...
\DIFaddend As alluded to above, consistency and reliability are the principal matter of concern.
...
results within the limits of statistical convergence.  \DIFaddbegin \DIFadd{Prior work has
successfully compared calculated absolute hydration free energies
across GROMACS and DESMOND codes.~\mbox{%DIFAUXCMD
\cite{klimovich_predicting_2010}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend This comparison has not yet been carried out \DIFaddbegin \DIFadd{for relative free energies}\DIFaddend .
...
(e.g.\DIFaddbegin \DIFadd{\ }\DIFaddend algorithms for pressure and temperature scaling, integrators,
\DIFaddbegin \DIFadd{cufoffs for Coulomb and vdW interactions, }\DIFaddend etc).  \DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{We also note that
the implementation of alchemical free energy calculations is very
different among the simulation codes (see Section~\ref{sec:afe_impl} for
details).  This implies that using the same free energy parameters across all codes, and especially using the same lambda schedule, will not automatically lead to equivalent free energies.
In Fig.~\ref{S-fig:ti_curves}x we show various plots of the free energy derivative versus lambda to demonstrate this.
Hence, the protocol and especially the choice of lambda values was adjusted individually for each code based
on previous experience of the researchers involved.  In }\DIFaddend addition there
may be \DIFdelbegin \DIFdel{difference }\DIFdelend \DIFaddbegin \DIFadd{differences }\DIFaddend in the choice of physical constants used for
...
GROMACS\DIFdelbegin \DIFdel{.\mbox{%DIFAUXCMD
\cite{Shirts2017,SOMDcoulomb}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Shirts2017, SOMDcoulomb}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend To circumvent some of these practical problems, we will compare
...
together all force field parameters \DIFaddbegin \DIFadd{simultaneously along the alchemical path --- }\DIFaddend i.e.\ partial charges, van der
Waals parameters, and bonded parameters\DIFdelbegin \DIFdel{vary
simultaneously along the alchemical path}\DIFdelend .  In the ``split protocol'' we calculate relative free
...
\DIFdelbegin \DIFdel{Another difference is how the codes scale force field parameters (``parameter
...
%DIFDELCMD < %%%
\DIFdel{One more }\DIFdelend \DIFaddbegin \DIFadd{One additional }\DIFaddend important issue is whether the code allows holonomic constraints to be applied
to bonds, which \DIFaddbegin \DIFadd{may }\DIFaddend change bond lengths in \DIFdelbegin \DIFdel{a transformation }\DIFdelend \DIFaddbegin \DIFadd{some transformations, }\DIFaddend e.g.\ C--H to C--C.
...
end states in one topology file.  \DIFaddbegin \DIFadd{In AMBER16 \progname{sander} and
\progname{pmemd} implement free energy simulations in an equivalent
...
the user can request that a softcore potential be used.  
\DIFdelbegin \DIFdel{The
non--bonded interactions of atoms }\DIFdelend \DIFaddbegin \DIFadd{Atoms }\DIFaddend that are not paired between the
end--states are \DIFdelbegin \DIFdel{handled with a softcore potential.  In addition,
bonded terms involving different unpaired atoms are ignored.  This in
effect amounts to defining unpaired atoms }\DIFdelend \DIFaddbegin \DIFadd{are effectively treated }\DIFaddend as dummy atoms in one of the
\DIFaddbegin \DIFadd{two }\DIFaddend end--states.  
\DIFaddbegin \DIFadd{Bonded terms involving different unpaired atoms are ignored
and their non--bonded interactions are handled with a softcore potential.
}\DIFaddend We call this the ``implicit dummy protocol'' since the
procedure is handled automatically by the software through analysis of 
the end--state topologies rather than via explicit \DIFdelbegin \DIFdel{definition }\DIFdelend \DIFaddbegin \DIFadd{introduction }\DIFaddend of dummy
atoms \DIFdelbegin \DIFdel{in an input topology }\DIFdelend \DIFaddbegin \DIFadd{that is required in computations based on a single topology framework}\DIFaddend .
...
%DIF <  BR: Personally, I would not call PERT a dual topology. In fact, it is neither completely dual nor single. It depends on the user.(RESOLVED)
...
terms are linearly scaled by the coupling parameter $\lambda$.  The \DIFdelbegin \DIFdel{PSSP softcore
potential }\DIFdelend \DIFaddbegin \DIFadd{softcore
potential (activated with the PSSP keyword and used here as identifier
in the further discussion, see the SI for implementation details) }\DIFaddend is
...
The code allows changing bond lengths involving constraints within the perturbed group  but this can lead to instabilities and wrong results (Michael Shirts, private communication).  There are separate \DIFdelbegin \DIFdel{$\lambda$s }\DIFdelend \DIFaddbegin \DIFadd{lambda values }\DIFaddend for LJ,
...
GROMACS, SOMD and NAMD~\cite{JCC:JCC20289} \DIFaddbegin \DIFadd{(NAMD is currently a purely
dual--topolgy code and has thus not been considered in this study due
to the technical differences as explained in the Introduction)}\DIFaddend .  The
...
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:cycles} shows all \DIFdelbegin \DIFdel{18 transformations}\DIFdelend \DIFaddbegin \DIFadd{9 transformations, run in forward and
backward directions, }\DIFaddend considered in the present study. In the limit of
...
\DIFdelend \begin{figure}[ht]
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=1.0]{figures/cycles.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/cycles.pdf}
  \DIFaddendFL \caption{The thermodynamic cycles considered in this study.  To
...
alternative mappings have been considered, see \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:cycles}.  One mapping has methane matched to a terminal methyl (green) and the other
...
%DIF < BR: NOT ONLY ABOUT RAFE IN THE FOLLOWING...(RESOLVED)
...
At the most basic level, \DIFdelbegin \DIFdel{even }\DIFdelend \DIFaddbegin \DIFadd{important differences could even result from }\DIFaddend pressure and temperature scaling, integrators and other algorithms\DIFdelbegin \DIFdel{can also display important differences}\DIFdelend .
It is unclear if and how any of these implementation details \DIFdelbegin \DIFdel{can }\DIFdelend affect results.
The implementation details of alchemical free energy simulation in code are discussed in subsection~\ref{sec:afe_impl}.

In this study we consider \DIFdelbegin \DIFdel{at }\DIFdelend a set of simple organic molecules (see
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:cycles}).  As the focus here is on probing for
...
1.8),~\cite{wang_development_2004} utilizing \DIFdelbegin \DIFdel{AM1/BCC }\DIFdelend \DIFaddbegin \DIFadd{AM1-BCC }\DIFaddend charges~ for the solute,\cite{jakalian_fast_2000,jakalian_fast_2002} and
...
\DIFaddbegin \DIFadd{All parameters and input files are available at
https://github.com/halx/relative-solvation-inputs.
}\DIFaddend The quality of free energies \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{estimated using }\DIFaddend various small molecule force fields
has been discussed elsewhere \DIFdelbegin \DIFdel{, see e.g. Refs.~\mbox{%DIFAUXCMD
\citenum{doi:10.1021/ct300203w,Hu2016}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{and is not a focus of this work; here we focus on reproducibility given a particular force field.~\mbox{%DIFAUXCMD
\cite{doi:10.1021/ct300203w}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend 

While the MD packages \DIFaddbegin \DIFadd{employed }\DIFaddend principally allow a ``one--step''
transformation~\cite{steinbrecher_soft-core_2011},
that is\DIFaddbegin \DIFadd{, }\DIFaddend with both LJ and Coulombic parameters varied
simultaneously (\DIFaddbegin \DIFadd{what we call a }\DIFaddend unified protocol), it has also been proposed that carrying out a
...
equilibrated but the final perturbed system \DIFdelbegin \DIFdel{combined from those , }\DIFdelend \DIFaddbegin \DIFadd{is a composite of coordinates from those }\DIFaddend potentially
differently sized solutes.  To make the number of atoms the same for forward
and backward setups\DIFaddbegin \DIFadd{, }\DIFaddend the water coordinates of the larger of the two boxes are
chosen.  Thus, in transformations from a smaller to a larger solute, water
molecules may be in close proximity to the solute.  \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{At the end of the
construction process, FESetup performs a minimization onto the
system.  In addition, some simulation protocols started with an additional (
redundant) minimization step.  All }\DIFaddend production simulations were run at
\SI{298}{K} and \SI{1.0}{bar} \DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{in the NPT ensemble.  Water molecules
were constrained.  }\DIFaddend Atomic masses were not changed along the
...
\DIFaddbegin \DIFadd{A summary of the main algorithmic differences between each simulation package is given in
...
We used a \DIFdelbegin \DIFdel{20-step alchemical protocol where }\DIFdelend \DIFaddbegin \DIFadd{20-window alchemical protocol with five windows for }\DIFaddend charge coupling and \DIFdelbegin \DIFdel{LJ coupling were dealt with separately along the path \mbox{%DIFAUXCMD
\cite{Mobley2014, doi:10.1021/acs.jced.7b00104}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{fifteen windows for LJ coupling \mbox{%DIFAUXCMD
\cite{Mobley2014, doi:10.1021/acs.jced.7b00104}}\hspace{0pt}%DIFAUXCMD
.
Our choice allows soft core potentials to be used only when changing nonpolar interactions, and allows electrostatic interactions to be changed linearly \mbox{%DIFAUXCMD
\cite{naden_linear_2015}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend %
...
No holonomic bond or angle constraints \DIFaddbegin \DIFadd{for the solutes }\DIFaddend were used. \DIFaddbegin \DIFadd{Waters were constrained with LINCS. 
}\DIFaddend A Parrinello--Rahman barostat with $\tau_p =$ \SI{10}{ps} and compressibility equal to \SI{4.5d-5}{bar^{-1}} was used.
%
...
%Starting coordinates of the windows
The starting coordinates for each window were obtained by \DIFaddbegin \DIFadd{an additional }\DIFaddend energy minimization of the same pre-equilibrated \DIFaddbegin \DIFadd{and minimized }\DIFaddend configuration generated by FESetup.
%Simulation length timestep
A velocity-Verlet integrator was employed with a \SI{2}{fs} time step.
%SHAKE and CONSTRAINTS
\DIFdelbegin \DIFdel{Only }\DIFdelend \DIFaddbegin \DIFadd{Water hydrogens (TIP3P) were constrained with SHAKE. For the alchemical solute, only }\DIFaddend bonds involving hydrogens which are not alchemically transformed were constrained. This approach is referred as the ``unperturbed H bond constraint protocol''. \DIFaddbegin \DIFadd{Given the number of the perturbed hydrogen bonds in the solutes (Fig.~\ref{fig:cycles}, this constraint allows to use a \mbox{%DIFAUXCMD
\SI{2}{fs} }\hspace{0pt}%DIFAUXCMD
time-step through use of higher atomic masses for perturbed hydrogen atoms (see discussion below).
}\DIFaddend %Temperature and pressure
...
\centering
\caption{\DIFaddFL{Summary of the technical details for the relative hydration free energy calculations carried out with the various codes.}}
\label{tab:resumeSpecifications}
\DIFaddFL{\makebox[\textwidth][c]{
\resizebox{0.7\width}{!}{
\begin{tabular}{lllll}
\hline
...
\DIFaddend \subsection{Free Energy Estimations}
\label{sec:analysis}
...
given $\lambda$ value.  %DIF < An AFE simulation is typically carried
%DIF < out in a series of equilibrium simulations at discrete values of $\lambda$ but
%DIF < the gradient can also be evaluated with a continuously varying coupling
%DIF < parameter as a function of the simulation time.  
\DIFdelbegin \DIFdel{The free energy is finally
computed through a suitable numerical integration method. }\DIFdelend Results from additional
...
TI with cubic splines, BAR and MBAR.  \DIFaddbegin \DIFadd{We have used the cubic splines
method to integrate the free energy.  }\DIFaddend All data was sub--sampled to
eliminate correlated data\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{doi:10.1021/ct0502864}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
computed as the average for each direction separately.  For comparison we have also calculated the absolute (standard) hydration free energies for all molecules in \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:cycles}.
...
\end{equation}
where $\sigma$ is the sample standard deviation \DIFdelbegin \DIFdel{and $n$ is the number of uncorrelated samples, as computed by the }\texttt{\DIFdel{pymbar}} %DIFAUXCMD
\DIFdel{module}\DIFdelend \DIFaddbegin \DIFadd{of the three $\Delta \Delta G_{\mathrm{hydr}}$ values, and $n=3$}\DIFaddend .
%DLM: Add ref
\DIFdelbegin \DIFdel{The SEM for component free energies is combined }\DIFdelend \DIFaddbegin \DIFadd{For each free energy change the SEM was evaluated }\DIFaddend as:
\begin{equation}
  \label{eq:sem-comb}
...
\begin{equation}
\label{eq:MUE}
\mathrm{MAE} = \DIFdelbegin \DIFdel{\frac{1}{n}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{N}}\DIFaddend \sum_\mathrm{i=1}\DIFdelbegin \DIFdel{^n }\DIFdelend \DIFaddbegin \DIFadd{^N }\DIFaddend \left | y_\mathrm{i} -
x_\mathrm{i} \right |
\end{equation}
where \DIFdelbegin \DIFdel{$n$ }\DIFdelend \DIFaddbegin \DIFadd{$N$ }\DIFaddend is the total number of samples, $y_\mathrm{i}$ and $x_\mathrm{i}$ are
...
\DIFaddbegin \subsection{\DIFadd{Overall comparison}}
\label{sec-comparison}

\DIFadd{In the following we will present our RAFE results for the
thermodynamic cycles shown in Fig.~\ref{fig:thermocycle}.  }\DIFaddend We will use
...
high precision\DIFdelbegin \DIFdel{,~\mbox{%DIFAUXCMD
\cite{doi:10.1021/acs.jced.7b00104}}\hspace{0pt}%DIFAUXCMD
%DIF < check for more references
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{doi:10.1021/acs.jced.7b00104}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend and are simpler to
set up and implement than relative calculations.  
\DIFdelbegin \DIFdel{Prior work has successfully compared calculated absolute hydration
free energies across GROMACS and DESMOND codes.~\mbox{%DIFAUXCMD
\cite{klimovich_predicting_2010}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend 

\DIFdelbegin \DIFdel{Table~}\DIFdelend \DIFaddbegin \DIFadd{Tab.~}\DIFaddend \ref{tab:absolute2} summarizes results for the absolute hydration
free energies.  The table shows the data
from simulations with the \DIFdelbegin \DIFdel{recommended protocol for each MD code }\DIFdelend \DIFaddbegin \DIFadd{protocol our groups considered most trustworthy for the respective MD code used}\DIFaddend , as discussed in detail in the following subsections.
...
methanol by about \SI{+0.2}{kcal.mol^{-1}}. 
\DIFdelbegin \DIFdel{Similarly, there are some
...
(typically smaller by ca.\ \mbox{%DIFAUXCMD
\SI{0.01}{g/cm^3}}\hspace{0pt}%DIFAUXCMD
).
}\DIFdelend The largest deviation can be found for one of the largest
...
post-processing of trajectories.  For reference, a recent study from \DIFdelbegin \DIFdel{Lee--Ping }\DIFdelend \DIFaddbegin \DIFadd{Wang }\DIFaddend et al.\ reports a
...
\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:mue-absolute} shows the \DIFdelbegin \DIFdel{MAD }\DIFdelend \DIFaddbegin \DIFadd{MAE }\DIFaddend between SOMD, GROMACS, AMBER and CHARMM. CHARMM produces figures that agree the most with other MD packages. The largest difference reaches \SI{0.2}{kcal.mol^{-1}} for SOMD and GROMACS. Variabilities between the codes may be partly explained by differences in densities due to different treatments of long range electrostatics and vdW interactions.

%DIF < {\footnotesize \bf \color{red} [BR: I am not sure how Table~%\ref{tab:mue-absolute} was generated. I insert my own table below based on Table \ref{tab:absolute2} and the deviations are not smaller for SOMD.]}
%DIF < JM resolved this as using previous CHARMM and GROMACS-reaction field values
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \begin{table}[h!]
%DIF < {\color{red}
%DIF <     \caption{\color{red} BR: MATRIX OF ABSOLUTE DEVIATIONS %BETWEEN PROGRAMS}
%DIF <       \begin{tabular}{l|cccc}
%DIF <         & AMBER  &  CHARMM  & GROMACS &  SOMD  \\ \hline
%DIF < AMBER   & 0.000  &   0.126  &  0.198  &  0.136 \\
%DIF < CHARMM  & 0.126  &   0.000  &  0.148  &  0.066 \\
%DIF < GROMACS & 0.198  &   0.148  &  0.000  &  0.209 \\
%DIF < SOMD    & 0.136  &   0.066  &  0.209  &  0.000 \\
%DIF <       \end{tabular}
%DIF < }
%DIF < \end{table}
%DIFDELCMD < 
...
\DIFdelend \begin{table}[]
 \begin{minipage}{\linewidth}
   \caption{Mean Absolute \DIFdelbeginFL \DIFdelFL{Deviations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Error }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{MAD}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MAE}\DIFaddendFL ) (\SI{}{kcal.mol^{-1}}) between relative free energies obtained with the absolute protocol for the SOMD, GROMACS, AMBER and CHARMM packages.}\label{tab:mue-absolute}
   \makebox[\textwidth][c]{
     \begin{tabular}{lrrr}
       \toprule
...
\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:relative} summarizes the results for the four MD packages.
...
{\footnotesize
  \begin{minipage}{\linewidth}
    \caption{Comparison of relative free energies of hydration for various MD
      ...
      protocols.  The values deduced from AFE transformations (given in the first row) were taken from \DIFdelbeginFL \DIFdelFL{Table }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Tab. }\DIFaddendFL 1.
...
terminal mapped neopentane case reveals AMBER to be in line with GROMACS and SOMD while CHARMM's results deviate further.  AMBER deviates also quite strongly from the other codes in the cyclopentanyl indole cases. \DIFaddbegin \DIFadd{It is possible that the discrepancies observed with AMBER are partly due to inconsistencies in the end point geometries (see section \ref{sec:amber-results}).
}\DIFaddend 

The \DIFdelbegin \DIFdel{MADs }\DIFdelend \DIFaddbegin \DIFadd{MAEs }\DIFaddend of the relative free energy simulations are presented in
\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:mue-relative}.  They are on average slightly larger than the \DIFdelbegin \DIFdel{MADs }\DIFdelend \DIFaddbegin \DIFadd{MAEs }\DIFaddend from the absolute simulations (\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:mue-absolute}) and reach \SI{0.26}{kcal.mol^{-1}} for AMBER compared with CHARMM.

\begin{table}[]
 \begin{minipage}{\linewidth}
   \caption{\DIFdelbeginFL \DIFdelFL{MAD }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MAE }\DIFaddendFL (in \si{kcal.mol^{-1}}) comparing relative free energies from
...
We also computed cycle closure errors from \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:cycle-closure} for
the closed cycle ethane$ \rightarrow$ methanol $\rightarrow$ methane $\rightarrow$ ethane (see \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:cycles}). The results are shown in \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:cycle-closure}. Uncertainties were estimated by propagating uncertainties from the individual perturbations. \DIFdelbegin \DIFdel{AMBER explicit dummy and CHARMM are the only protocol }\DIFdelend \DIFaddbegin \DIFadd{The AMBER protocols, CHARMM and SOMD are }\DIFaddend consistent within uncertainty estimates, but the deviations observed with the \DIFdelbegin \DIFdel{other }\DIFdelend \DIFaddbegin \DIFadd{GROMACS }\DIFaddend protocols are small. The largest discrepancy is observed with the GROMACS unified PME protocol, with the error just under \SI{0.2}{kcal/mol}.

\begin{table}[]
  \begin{minipage}{\linewidth}
    \caption{Cycle closure errors \DIFdelbeginFL \DIFdelFL{(}\DIFdelendFL in \si{kcal.mol^{-1}}) for  ethane$ \rightarrow$ methanol\DIFaddbeginFL \DIFaddFL{. }\DIFaddendFL $\rightarrow$ methane
$\rightarrow$ ethane\DIFaddbeginFL \DIFaddFL{. Uncertainties denote a 95\% confidence  interval.}\DIFaddendFL }\label{tab:cycle-closure}
    \makebox[\textwidth][c]{
      \begin{tabular}{lrrr}
        \toprule
        Package and Protocol & Closure Error \\
        \midrule
        AMBER implicit    & 0.07 $\pm$ \DIFdelbeginFL \DIFdelFL{0.04  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.08  }\DIFaddendFL \\
        AMBER explicit    & 0.02 $\pm$ \DIFdelbeginFL \DIFdelFL{0.05  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.10  }\DIFaddendFL \\
        GROMACS split reaction field  &  0.05 $\pm$ \DIFdelbeginFL \DIFdelFL{0.02  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.04  }\DIFaddendFL \\
        GROMACS unified reaction field  &  0.13 $\pm$ \DIFdelbeginFL \DIFdelFL{0.03  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.06  }\DIFaddendFL \\
        GROMACS split PME  &  0.04 $\pm$ \DIFdelbeginFL \DIFdelFL{0.01  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.02  }\DIFaddendFL \\
        GROMACS unified PME  &  0.18 $\pm$ \DIFdelbeginFL \DIFdelFL{0.03  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.06  }\DIFaddendFL \\
        CHARMM            &   0.01 $\pm$ \DIFdelbeginFL \DIFdelFL{0.03   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.06   }\DIFaddendFL \\
        SOMD                &  -0.11 $\pm$ \DIFdelbeginFL \DIFdelFL{0.08 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.16 }\DIFaddendFL \\
        \bottomrule
      \end{tabular}
    }
  \end{minipage}
...
Finally we also examined whether the codes reproduced consistent changes in mean box volumes between forward and backward transformations. We find that the codes are generally consistent with GROMACS giving the most precise volume changes, whereas SOMD gives the least precise volume changes (See \DIFdelbegin \DIFdel{Table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend S1 in the SI). This indicates that the barostats used by the different simulation packages relax volume fluctuations with different efficiency, or that they sample different volume fluctuations.

\DIFaddbegin \DIFadd{We now turn to considerations for individual packages. 
}

\DIFaddend \subsection{AMBER}
\label{sec:amber-results}

Using AMBER for RAFE simulations has revealed several problems with the implementation.  Some \DIFdelbegin \DIFdel{bugs }\DIFdelend \DIFaddbegin \DIFadd{issues }\DIFaddend were identified and the developers have fixed those for AMBER16, e.g.\ energy minimization in \progname{sander} led to
...
\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{S-tab:amber-onestep}x). The issue becomes apparent when more than a few dummy atoms are involved, while the unified protocol works for the
smaller transformations (refer to \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:cycles}).  The split RAFE
protocol and absolute free energies, however, are very close to the other MD
packages as demonstrated in \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:amber-comp} below.

End point geometries appear to be another issue with AMBER simulations \DIFdelbegin \DIFdel{in both }\DIFdelend \DIFaddbegin \DIFadd{both in }\DIFaddend solution and vacuum.  This is most obvious in the neopentane
$\rightarrow$ methane test case with central mapping (see
\nameref{sec:rafe_setup} and \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{fig:thermocycle}).
As shown in \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{S-fig:amber-dist}x, the methane end state exhibits
...
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{S-fig:amber-dist}x demonstrates how this depends on the number of
...
absolute transformations described in \DIFdelbegin \DIFdel{Tables}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:absolute2} and
...
region in these protocols.  \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:amber-comp} shows selected results
...
based mask for \DIFdelbegin \DIFdel{bond SHAKEs }\DIFdelend \DIFaddbegin \DIFadd{application of bond constraints }\DIFaddend such that the mask must be set for the hydrogens in
question while the same is not possible for their non--H counter--part in the
other state because \emph{all} bonds emanating from \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend atom would be affected.
...
(\DIFdelbegin \DIFdel{Tables}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:absolute2} and \ref{tab:relative}).  There are, however, a few
...
transformations (\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:absolute2}).  We also observe a systematic
deviation between forward and backward vacuum transformations in the
2--methylindole simulation (see \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{S-tab:amber-disc}x).  The gradient
...
CHARMM for alchemical free energy calculation (AFE) has been widely used with \DIFaddbegin \DIFadd{the }\DIFaddend PERT module, but few \DIFdelbegin \DIFdel{bugs }\DIFdelend \DIFaddbegin \DIFadd{issues }\DIFaddend not previously reported in CHARMM c40b1 were found and careful AFE setup is needed to produce robust and accurate results. Bugs regarding TI gradient accumulation in the parallel version were identified and fixed by Dr. Stefan Boresch.
The PERT module does not allow a hydrogen bond constraint (SHAKE) to be applied on the perturbed region, and this requires end point lambdas to be equilibrated carefully. These windows at end-point lambda were started with their own equilibration using timesteps of \SI{0.1}{fs} to \SI{0.5}{fs} before the production run. The VSwitch option was used to apply a switching function to the potential since that option \DIFdelbegin \DIFdel{is }\DIFdelend cannot be applied to forces for calculations run with the PERT module.
...
7-CPI & 2-CPI & \num{-0.01+-0.01} & \num{0.04+-0.01} &  \\
  ...
  \end{minipage}
\end{table}
While results from all three protocols (split, unified, absolute) seem to be in good agreement with each other, the split-protocol results are more precise due to the additional amount of data generated. It is notable that the split-protocol results are more similar to the ones obtained by other MD packages (i.e. neopentane and toluene), but the relative-unified results are more consistent with the CHARMM absolute simulations (e.g. 2-methylindole). Overall, the relative free energies obtained by these three different protocols are in good agreement with those reported for the other MD packages (\DIFdelbegin \DIFdel{Tables }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend 1 and 3).
...
in the \inpopt{.mdp} file.  
\DIFaddbegin 

\DIFadd{Here, we use these options to simplify our setup.
}\DIFaddend For instance, in split protocol simulations, these
...
\DIFdelbegin \DIFdel{Table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend \ref{tab:groresults} lists the relative free energies obtained from
GROMACS simulations.
\begin{sidewaystable}[]
  \centering
...
%DIF < BR: The foolling around with masses of dummy atoms is mostly a technical trick. The whole thing could have been run with a mass of 5 AMU on everyone....
%DIF <  We should make it clear that free energy do not depend on masses....
%DIF < JM clarified below
To achieve this level of reproducibility within SOMD it was crucial to pay close attention to constraints. Specifically, bonds that involve unperturbed hydrogen atoms are constrained. Bonds involving hydrogen atoms that are perturbed to a heavy element are unconstrained.  Additionally the atomic mass of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{any }\DIFaddend perturbed hydrogen atom is set to the mass of the heavy atom it is \DIFaddbegin \DIFadd{being
}\DIFaddend perturbed to.  Bonds involving hydrogen atoms that are perturbed to another hydrogen atom type are constrained. We stress that it is acceptable to artificially increase the atomic mass of hydrogen atoms because the calculated excess free energy changes do not depend on atomic masses.
...
\SI{0.09}{kcal.mol^{-1}}) to those computed from simulations where \DIFdelbegin \DIFdel{noconstraints are applied on the }\DIFdelend \DIFaddbegin \DIFadd{no constraints are applied for }\DIFaddend solutes and a timestep of \SI{1}{fs} is used (See \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig.}\DIFaddend ~\ref{S-fig:sire_constraints}x).
...
\DIFdelbegin \DIFdel{tab}\DIFdelend \DIFaddbegin \DIFadd{Tab}\DIFaddend .~\ref{S-tab:sire_constraints}x and fig.~\ref{S-fig:sire_constraints}x.
...
shifted Barker Watts,~\cite{doi:10.1080/00268977300102101} \DIFdelbegin \DIFdel{vs group based }\DIFdelend \DIFaddbegin \DIFadd{versus group-based }\DIFaddend switched Barker Watts), but nevertheless SOMD and GROMACS RF produce comparable results with a \DIFdelbegin \DIFdel{MAD }\DIFdelend \DIFaddbegin \DIFadd{MAE }\DIFaddend of \SI{0.18}{kcal.mol^{-1}}. Overall, the SOMD free energy estimations are in good agreement with the other MD packages, as the \DIFdelbegin \DIFdel{MAD }\DIFdelend \DIFaddbegin \DIFadd{MAE }\DIFaddend suggests (see \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:mue-relative}). For the methane $\rightarrow$ neopentane transformations SOMD yields consistent results between central and terminal mappings, as shown in \DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{S-tab:sire-finalresults}x.
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This was the case despite our best efforts to maintain fairly consistent protocols for settings which were expected to significantly impact results.
...
Overall, the }\DIFaddend MD codes have a wide range of options and setup features which makes it difficult for the \DIFdelbegin \DIFdel{unexperienced }\DIFdelend \DIFaddbegin \DIFadd{inexperienced }\DIFaddend user to decide on the most
appropriate ones.

The free energies we have computed appear to be in reasonably good agreement with each other (see \DIFdelbegin \DIFdel{Tables}\DIFdelend \DIFaddbegin \DIFadd{Tab.}\DIFaddend ~\ref{tab:absolute2} and \ref{tab:relative}).  The average MAE between all codes \SI{0.14}{kcal/mol} for absolute free energies and \SI{0.17}{kcal/mol} for relative free energies.  This can be interpreted as the current ``limit of reproducibility'' for the field.  We have found viable protocols for each MD code to achieve
...
geometries.  This is particularly evident in the \DIFaddbegin \DIFadd{case of the transformation from }\DIFaddend neopentane to methane
\DIFdelbegin \DIFdel{case
}\DIFdelend with central mapping\DIFdelbegin \DIFdel{where also }\DIFdelend \DIFaddbegin \DIFadd{, where }\DIFaddend the relative free energies are clearly
different from the other packages.  We suspect these \DIFdelbegin \DIFdel{issues reflect a bug }\DIFdelend \DIFaddbegin \DIFadd{reflect issues }\DIFaddend in
the AMBER package but have been unable to isolate it; we have reported the
issue to the AMBER developers.

We were unable to define  a \emph{universal} protocol that could be recommended for use with all four codes.  Unified protocols do not appear to work \DIFaddbegin \DIFadd{adequately }\DIFaddend with AMBER and GROMACS while SOMD and CHARMM had no problem in this regard.  We cannot rule out that the problem may lie e.g.\ only with the
...
to work well for all codes.  It appears to be the most \DIFdelbegin \DIFdel{efficient
}\DIFdelend \DIFaddbegin \DIFadd{effective
}\DIFaddend approach for GROMACS as shown with the methanol to methane case
...
%DIF <  HHL: is this
%DIF < really true?  Procacci discusses this in the context of fast switching.

Comparison between codes is hampered by several factors.  Firstly, the codes use different simulation algorithms \DIFaddbegin \DIFadd{--- }\DIFaddend e.g.\
electrostatics are handled differently in vacuum i.e.\ infinite cutoff vs.\
reaction field.  Temperature and pressure control, time step\DIFaddbegin \DIFadd{, }\DIFaddend integrators, etc. are other examples.  But the data here suggest that, if there are any systematic errors introduced through these algorithms, then they are small.  It is reassuring that AFEs for the systems tested here show only a small dependence on MD protocol decisions (provided a correct implementation).

\DIFdelbegin \DIFdel{As part of this work }\DIFdelend \DIFaddbegin \DIFadd{Some of the differences between protocols used in this comparison could have been avoided, and it may be worth pursuing further harmonizing the protocols in follow up work.
...
This could also allow for a direct comparison of efficiency across codes.
}

\DIFadd{To aid with follow-up studies, }\DIFaddend we make our input data and protocols available.  We recommend using this dataset to test and benchmark future RAFE implementations to validate reproducibility against other simulation packages.
...
The relative transformation should be run in both forward and backward directions, even if the free energy estimator is agnostic to this decision, as other implementation details (e.g. \DIFdelbegin \DIFdel{bugs in }\DIFdelend parameters, atomic masses\DIFaddbegin \DIFadd{, use of bond constraints, or details of the number of dummy atoms and how they are handled}\DIFaddend ) may lead to inconsistent results \DIFaddbegin \DIFadd{depending on the transformation direction}\DIFaddend .

More specifically, various issues with current code bases have been revealed through this work.
We have found that constraints in connection with varying bond length can cause errors with GROMACS, just as masses \DIFaddbegin \DIFadd{(in many codes) }\DIFaddend must not be allowed to vary in RAFE simulations, both to avoid crashes and incorrect results from the
software.  CHARMM has \DIFdelbegin \DIFdel{issues with }\DIFdelend \DIFaddbegin \DIFadd{problems handling }\DIFaddend constraints and the PSSP softcores, and the PERT module cannot make use of the force switch as \DIFdelbegin \DIFdel{it }\DIFdelend is now standard for CHARMM force fields. Care must be taken when using the LRC long range correction keyword to avoid producing inconsistent results.  AMBER's problem with end point geometries and unified protocols has been pointed out above.
...
\DIFdelbegin \DIFdel{Another practical issue is the complex setup associated with the split protocol}\DIFdelend \DIFaddbegin \DIFadd{In general we found that split protocols perform well, but these can necessitate complex steps to set up the calculations}\DIFaddend .
For instance\DIFaddbegin \DIFadd{, }\DIFaddend in GROMACS it is necessary to carry out two separate simulations per lambda because discharging and recharging groups cannot be selected separately.
Lambda paths as implemented in GROMACS could also be beneficial for other codes as they make the setup of split protocols easier.  The alternative we have used in codes lacking this feature is to \DIFdelbegin \DIFdel{mimick }\DIFdelend \DIFaddbegin \DIFadd{mimic }\DIFaddend this protocol through careful \DIFdelbegin \DIFdel{constructions }\DIFdelend \DIFaddbegin \DIFadd{construction }\DIFaddend of
topologies via scripting.

\DIFaddbegin \DIFadd{It may seem remarkable that of the computed free energies, the absolute hydration free energies seem to be more reproducible across codes than relative free energies.
Conventional wisdom is that relative free energy calculations are computationally less demanding than absolute free energy calculations, which tend to lead to the opposite result~\mbox{%DIFAUXCMD
\cite{christ_basic_2010, Michel2010, Liu2013}}\hspace{0pt}%DIFAUXCMD
.
However, absolute calculations are considerably simpler to implement and deploy correctly as they do not involve as many challenging technical issues such as atom mapping~\mbox{%DIFAUXCMD
\cite{Liu2013, Cournia:2017:J.Chem.Inf.Model.}}\hspace{0pt}%DIFAUXCMD
, and the lambda protocols which must be employed have been optimized fairly well, since such calculations always involve either removal or insertion of atoms but never both simultaneously.
This has made absolute calculations valuable as large-scale tests of free energy methods and force fields (e.g.~\mbox{%DIFAUXCMD
\cite{Mobley2014, doi:10.1021/ct300203w, doi:10.1021/acs.jced.7b00104} }\hspace{0pt}%DIFAUXCMD
and others), and in the SAMPL series of blind challenges (e.g.~\mbox{%DIFAUXCMD
\cite{fennell_fixed-charge_2014, geballe_sampl3_2012, Mobley2014}}\hspace{0pt}%DIFAUXCMD
).
Thus absolute calculations are already well automated, robust across codes (e.g.~\mbox{%DIFAUXCMD
\cite{klimovich_predicting_2010}}\hspace{0pt}%DIFAUXCMD
), and well-performing protocols are available.
Apparently similar is yet needed for relative free energy calculations.
}

\DIFaddend The primary focus of this work was to achieve low statistical errors
to establish if codes are able to reproduce free energies.  We have
not investigated \DIFdelbegin \DIFdel{in detail }\DIFdelend the efficiency of the respective protocols
as this would require further, complex investigations.  \DIFaddbegin \DIFadd{As noted, the work reported here used different protocols and in some cases even different numbers of simulation steps for each code.
Thus a direct comparison of efficiency is outside the scope of this work.
However, it is worth briefly noting the number of steps employed in each study.
}\DIFaddend For absolute
calculations the most demanding \DIFdelbegin \DIFdel{protocol and most }\DIFdelend \DIFaddbegin \DIFadd{and (perhaps not surprisingly) }\DIFaddend precise protocol is
GROMACS (200 million aggregate time--steps per solute, average SEM
\SI{0.011}{kcal/mol})\DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{.
The }\DIFaddend least demanding protocol is CHARMM (31.5 million
...
\SI{0.048}{kcal/mol}).  \DIFdelbegin \DIFdel{Remarkably the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend most demanding protocol (GROMACS
197.4 million time--steps, average SEM \SI{0.020}{kcal/mol}) is less precise
than \DIFdelbegin \DIFdel{CHARMM that }\DIFdelend \DIFaddbegin \DIFadd{the CHARMM protocol which }\DIFaddend used fewer time--steps (31.5 million time-steps,
...
important for the efficiency of the free energy calculations. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < average SEM for absolute calcs in table 2 and aggregate number of MD steps per pert (1M = 1million)
%DIF <  AMBER 0.014 kcal/mol 80M steps
%DIF <  CHARMM 0.017 kcal/mol 31.5 M steps
%DIF <  GROMACS 0.011 kcal/mol 200M steps
%DIF <  SOMD 0.045 kcal/mol  34M steps
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < average SEM for relative calcs in table 4
%DIF <  AMBER implicit 0.024 kcal/mol 80M
%DIF <  AMBER explicit  0.027 kcal/mol 80M
%DIF <  CHARMM 0.015  kcal/mol  31.5M
%DIF <  GROMACS 0.020 kcal/mol  197.4M
%DIF <  SOMD 0.048 kcal/mol 17M
\DIFdelend \DIFaddbegin \DIFadd{In particular it may be interesting to apply overlap measures to explore the relative efficiency of the different protocols.~\mbox{%DIFAUXCMD
\cite{klimovich_guidelines_2015}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend 
